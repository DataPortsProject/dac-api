...
module.exports.createFilesForZIP = function createFilesForZIP(query) {
	...
	let folderDirectory = './src/zips/' + folderName;
	// -- Create folder --
	...
	fs.readFile(query.constants, 'utf8', function (err, data){
		...
		var fileUpdated = data.replace(/parameter_timeInterval/gim, query.time_interval);
		var timeUnit_newValue = fileUpdated.replace(/parameter_timeUnit/gim, '"' + query.time_unit + '"');
	...
	fs.readFile(query.script, 'utf8', function (err, data){
		...
		var orionUrlParameter = urlParameter.replace(/parameter_urlORION/gim, query.orion_url);
		var orionPortParameter = orionUrlParameter.replace(/parameter_orionPORT/gim, query.orion_port);
		var dataProviderParameter = orionPortParameter.replace(/parameter_dataProvider/gim, query.data_provider);
